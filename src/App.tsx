...
function App() {
  ...
  const [newTeamName, setNewTeamName] = useState<string>('');
   ...
   const [teamMessages, setTeamMessages] = useState<{ message: string; team: string; username: string }[]>([]);
   ...
   useEffect(() => {
     ...
     socket.on('teamMessage', (message: { message: string; team: string; username: string }) => {
         ...
         <div className="flex w-full max-w-6xl">
           ...
           <div className="w-1/3 bg-gray-50 rounded-lg shadow-md p-4">
             ...
             {Object.keys(lobbyState.teams).length > 0 ? (
               <ul>
                 {Object.entries(lobbyState.teams).map(([teamName]) => (
                   <li key={teamName} className="flex justify-between items-center mb-2">
                     <span className="text-green-500">{teamName}</span>: {lobbyState.teams[teamName]?.length} membre(s)
                     <button 
               ...
               <p>Pas de teams dans ce lobby.</p>
             )}
            ...
            <div className='mt-4'>
              ...
              <ul className='max-h-[200px] overflow-y-auto'>
                {teamMessages.map((msg, index) => (
                  <li key={index} 
                      className={`mb-2 ${msg.team === getCurrentUserTeam() ? 'text-green-600' : 'text-gray-600'}`}>
